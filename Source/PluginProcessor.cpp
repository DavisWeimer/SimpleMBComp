...
void SimpleMBCompAudioProcessor::prepareToPlay (double sampleRate, int samplesPerBlock)
{
    ...
    spec.maximumBlockSize = samplesPerBlock; //maximum amount of samples that can be processed at any time
    spec.numChannels = getTotalNumOutputChannels(); //max number of channels
    spec.sampleRate = sampleRate; // Sample Rate yo
...
//  Index position 3 select for default ratio
    ...
    layout.add(std::make_unique<AudioParameterBool>(ParameterID{"Bypassed", 5},
                                                    "Bypassed",
                                                    false));
